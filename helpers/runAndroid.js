 ...
 * 18/8/18
 * added --custom-run flag, adding a command line $ gradlew app:assembleDebug
 * for RNN (Wix react-navite navigation) purposes
 * 
 * file location: C:\Users\DELL\Autodidact\Autodidact\node_modules\react-native\local-cli\runAndroid
 * 
    runAndroid(argv, config, {
        root: '',
        appFolder: 'app',
        appId: '',
        appIdSuffix: '',
...
const __DIRNAME = path.join(__dirname, 'node_modules\react-native\local-cli');
...
function buildAndRun(args) {
  ...
  const packageNameWithSuffix = getPackageNameWithSuffix(args.appId, args.appIdSuffix, packageName);
    ...
    console.log(chalk.blue('Custom Run in action!\nHold on!\n\n'))
    if (args.customRun) {
        startServerInNewWindow(args.port);
    }
...
function runOnAllDevices(args, cmd, packageNameWithSuffix, packageName, adbPath){
    try {
        ...
        if (args.customRun) {
            console.log(chalk.bold(
                `Running RNN custom code (cd android && ${cmd} app:assembleDebug)...`
            ));

            child_process.execFileSync(cmd, ['app:assembleDebug'], {
                stdio: [process.stdin, process.stdout, process.stderr],
            });
        }
...
module.exports = {
  ...
  options: [{
    ...
    parse: (val: string) => Number(val),
          ...
          command: '--custom-run',
          description: 'Adds a custom command to the flow',
      },],
};

...
export type MathTextItemProps<T extends boolean = boolean> = (T extends true
  ? Omit<MathViewProps, "math">
  : TextProps) & {
  value: string;
  isMath: T;
  Component?: MathView;
...
export type MathTextProps = Pick<
  MathTextRowProps,
  | "direction"
  | "containerStyle"
  | "renderItem"
...
export const InlineMathItem = React.memo(
  ({
    value,
    isMath,
    CellRendererComponent,
    ...
    const el = isMath ? (
      <Renderer {...props} math={value} resizeMode="contain" config={config} />
    ) : (
      <Text {...props} style={styles.textMiddle}>
        {value}
      </Text>
    );
    const container =
...
export const MathTextRow = React.memo(
  ({
    value,
    isMath,
    direction,
        ...
        _.flatten(
          _.map(_.split(value, /\$+/g), (value, i) => {
            if (isMath || i % 2 === 1) {
              ...
              return _.map(_.split(_.trim(value), " "), (value) => ({
                value,
                isMath: false,
                inline: true,
              }));
...
const MathText = React.memo(
  ({ value, renderRow, style, math, ...props }: MathTextProps) => {
    if (__DEV__ && value && math) {
      console.warn("MathText has received both `value` and `math` props");
    } else if (!value && !math) {
      __DEV__ &&
        console.warn("MathText: please provide `value` or `math` prop");
      return null;
    }
    const statements = useMemo(
      () =>
        _.split(_.replace(value || `$${math}$`, /\\(\(|\))/g, "$"), /\$\$/g),
      [value]
    );
